...
const int N = 5;  // Tamanho da matriz (deve ser 2^n + 1 para funcionar com diamond-square)
...
void diamond(int mat[N][N], int x1, int y1, int x2, int y2) {
    int cx = (x1 + x2) / 2;
    int cy = (y1 + y2) / 2;
...
void square(int mat[N][N], int x1, int y1, int x2, int y2) {
    int cx = (x1 + x2) / 2;
    int cy = (y1 + y2) / 2;
